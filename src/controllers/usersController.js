...
exports.users_list = async function(req, res) {
  ...
  try {
    ...
    res.render('users', { title: '管理员', navigate: navigate, message: '列出具有管理员权限的用户列表', users: users });
  } catch (err) {
    res.render('users', { title: '管理员', navigate: navigate, message: '错误', users: null, error: '返回用户列表时出错: ' + err });
  }
}
...
exports.password_post = async function(req, res) {
  ...
  if (errors) {
    ...
    const message = '已成功设置密码为用户 ' + req.body.username;
