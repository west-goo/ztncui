...
exports.network_delete_post = async function(req, res) {
  ...
  try {
    ...
    res.render('network_delete', {title: '删除网络', navigate: navigate, error: '删除网络时出错 ' + req.params.nwid + ': ' + err});
  }
};
...
exports.v4AssignMode = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('v4AssignMode', {title: 'IPv4分配模式', navigate: navigate, network: network});
  } catch (err) {
    res.render('v4AssignMode', {title: 'IPv4分配模式', navigate: navigate, error: '为网络应用V4分配模式时出错 ' + req.params.nwid + ': ' + err});
  }
}
...
exports.v6AssignMode = async function (req, res) {
  ...
  try {
    ...
    navigate.whence = '/controller/network/' + network.nwid;
    res.render('v6AssignMode', {title: 'IPv6分配模式', navigate: navigate, network: network});
  } catch (err) {
    res.render('v6AssignMode', {title: 'IPv6分配模式', navigate: navigate, error: '为网络应用V6分配模式时出错 ' + req.params.nwid + ': ' + err});
  }
}
...
exports.easy_get = async function(req, res) {
  ...
  try {
    ...
    res.render('network_easy', {title: '网络简易设置', navigate: navigate, error: '解析网络的详细信息时出错 ' + req.params.nwid + ': ' + err});
  }
}
  ...
  if (errors) {
    ...
    try {
      const network = await zt.network_easy_setup(req.params.nwid,
                                                  ...
                                                  ipAssignmentPools,
                                                  v4AssignMode);
      res.render('network_easy', {title: '网络简易设置', navigate: navigate, network: network, message: '网络设置成功'});
    } catch (err) {
      res.render('network_easy', {title: '网络简易设置', navigate: navigate, error: '解析网络的详细信息时出错 ' + req.params.nwid + ': ' + err});
    }
  }
